...
/*
	This program make socket batch by using select.
    So can get many other socket request using one select batch.	
*/
int TcpSockfd;
int UdpSockfd;
int UcoSockfd;
...
void ProcessUdpRequest() {
	...
	printf("Receive UDP request: %s......", msg.data);

	msg.type = MSG_REPLY;
	sprintf(msg.data, "This is a reply from %d.", getpid());
	if(sendto(UdpSockfd, (char *)msg.data,sizeof(msg),0, (struct sockaddr *)&cliAddr, cliAddrLen) < 0) {
		perror("sendto");
		exit(1);
	}
	printf("Replied.\n");
